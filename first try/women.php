...
<html>
<head>
	<title>First Try - Women Fashion</title>
...
<body style="padding-top:43px;" onload="cnt();">
	...
	<ul class="w3-navbar w3-collapse w3-black w3-large w3-top">
		...
		<?php 
			...
			{
 		?>
 			<li class='w3-hide-small w3-xlarge w3-margin-left'>First Try</li>
...
<div id="demo" class="w3-hide w3-hide-large w3-hide-medium">
  ...
  <center>
	...
	<?php
		$sql="SELECT DISTINCT type FROM data WHERE gender='Women'";
	...
	<?php
		$sql="SELECT DISTINCT brand FROM data WHERE gender='Women'";
		$res=mysqli_query($con, $sql) or die("Database query Error");
		while($row=mysqli_fetch_array($res))
		{
			$brand=$row['brand'];
	...
	<?php
		$sql="SELECT DISTINCT color FROM data WHERE gender='Women'";
		$res=mysqli_query($con, $sql) or die("Database query Error");
		while($row=mysqli_fetch_array($res))
		{
			$color=$row['color'];
...
<div id="login" class="w3-modal">
		<div class="w3-modal-content w3-card-8 w3-animate-zoom" style="max-width:400px">
		    <header class="w3-container w3-yellow">
			    ...
			    <div class="w3-section w3-container">
			        ...
			        <button class="w3-white w3-input w3-border w3-rest" style="width:10%; height:7%;" onmousedown="down();" onmouseup="up();"><i id="eye" class="glyphicon glyphicon-eye-open"></i></button></div>
	...
	function cnt() 
  {
    ...
    xmlhttp.open("GET","cntdata.php?g="+"Women",true);
    xmlhttp.send();
	}
	function logincheck()
	{
