...
<?php
		...
		?>
			...
			<div class='w3-third w3-margin w3-left w3-large'>
				...
				?>
					<div class='w3-margin'>
						<a class='w3-btn w3-hover-red' href="choice.php?rem=true&id=<?php echo $id; ?>">Remove Live Try</a>
					</div>
					<script>
				...
				<?php }else{ ?>
				<div class='w3-margin'>
					<a class='w3-btn w3-hover-green' href="#" onclick="document.getElementById('livetry').style.display='block'">Live Try</a>
				</div>
				<?php } ?>
				<div class='w3-margin'><button class='w3-btn w3-black w3-hover-blue' id=<?php echo $bid; ?> onclick='func(<?php echo $id; ?>,<?php echo $logged; ?>);'><i class='glyphicon glyphicon-shopping-cart'></i> Add to Cart</button></div>
			</div>
<?php
		}
?>
		...
		<div class="w3-modal-content w3-card-8 w3-animate-zoom" style="max-width:400px">
		    <header class="w3-container w3-yellow">
			    ...
			    <div class="w3-section w3-container">
			        ...
			        <button class="w3-white w3-input w3-border w3-rest" style="width:10%;" onmousedown="down();" onmouseup="up();"><i id="eye" class="glyphicon glyphicon-eye-open"></i></button></div>
	...
	<div id="waiting" class="w3-modal"><br><br><br><br><div class="w3-center"><i class='fa fa-spinner w3-spin w3-text-white' style='font-size:64px;'></i></div></div>
	<div id="livetry" class="w3-modal">
		<div class="w3-modal-content w3-card-8 w3-animate-zoom" style="max-width:500px">
		    <header class="w3-container w3-yellow w3-margin-bottom">
			  	<h1>Live Try</h1>
			    <div class="w3-center"><br>
			    	<span onclick="document.getElementById('livetry').style.display='none'" class="w3-closebtn w3-container w3-padding-8 w3-display-topright" title="Close Modal">&times;</span>
			    </div>
				</header>
			<form action="<?php $_SERVER['PHP_SELF'] ?>" method="POST" enctype="multipart/form-data">
				<label class="w3-input w3-border-0">
				<input type="file" value="Choose Image" accept="camera/*" class="w3-input" name="image"></label>
				<input type="submit" class="w3-input w3-hover-green" value="Submit" name="letstry">
			</form>
			<?php
			copy("C:\\xampp\\htdocs\\fasttry\\useruploads\\$img", "C:/xampp/htdocs/fasttry/useruploads/dress.png") or die("Copy Error");
			$target_dir = "useruploads/img/";
			$target_file = $target_dir ."img.jpg";
			if(isset($_POST["letstry"])) 
			{
			        ...
			        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) 
			        {
			        	...
			        	echo "
			        		<script>
			        			...
			        			setTimeout(function(){window.location='choice.php?id=$id';},5000);
			        		</script>";
			    	} 
			    	else 
			    	{
